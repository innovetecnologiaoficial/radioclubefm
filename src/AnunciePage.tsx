    ...
    <div className="min-h-screen bg-slate-50 font-sans selection:bg-[#ff3e5e]/20 text-slate-800">
      ...
      <section className="py-20 lg:py-28 max-w-7xl mx-auto px-4">
        ...
        <div className="space-y-24">
          {/* Cobertura */}
          <div className="flex flex-col lg:flex-row items-center gap-12 bg-white rounded-[3rem] p-8 lg:p-12 shadow-2xl border border-slate-100 relative overflow-hidden group">
            ...
            <div className="lg:w-1/2 relative z-10">
               <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#ff3e5e]/10 text-[#ff3e5e] font-bold text-sm mb-6">
                <Users className="w-4 h-4" />
                +150 Mil Habitantes
              </div>
              <h3 className="text-3xl md:text-4xl font-black text-[#5c337c] mb-4">Nossas Ondas Chegam Até Você!</h3>
              <p className="text-lg text-slate-600 mb-8 leading-relaxed">Cobertura ampla e qualificada englobando áreas residenciais e comerciais estratégicas. Conectando pessoas e histórias nos principais bairros de Criciúma e Içara.</p>
              <div className="flex flex-wrap gap-2 sm:gap-3">
                {['Próspera', 'Centro', 'Demboski', 'Liri', 'Raichaski', 'Pres. Vargas', 'Cristo Rei', 'Nª Sra da Salete', 'Jardim Silvana', 'Tereza Cristina', 'Jaqueline', '1º de Maio'].map((bairro) => (
                  <span key={bairro} className="bg-slate-100 text-slate-700 px-3 py-1.5 sm:px-4 sm:py-2 text-xs sm:text-sm rounded-full font-semibold border border-slate-200">
                    {bairro}
                  </span>
                ))}
              </div>
            </div>
            <div className="lg:w-1/2 w-full relative z-10 flex items-center">
              <div className="w-full relative aspect-[4/3] sm:aspect-video lg:aspect-[4/3] rounded-[2rem] overflow-hidden shadow-2xl border-4 border-white/20 group">
                {/* Background Image (Criciúma) */}
                <img 
                  src="https://wsrv.nl/?url=viva.criciuma.sc.gov.br/img/1744297907-Mirante-Realdo-Santos-Guglielmi.png" 
                  alt="Criciúma" 
                  className="absolute inset-0 w-full h-full object-cover pointer-events-none"
                  referrerPolicy="no-referrer"
                />
                
                {/* Foreground Image (Içara) - Clipped with clip-path */}
                <img 
                  src="https://www.icara.sc.gov.br/municipio/galeria_img/01_nova.jpg" 
                  alt="Içara" 
                  className="absolute inset-0 w-full h-full object-cover pointer-events-none"
                ...
                {/* Visual Divider / Slider Handle */}
                <div 
                  className="absolute top-0 bottom-0 w-1 bg-white shadow-[0_0_10px_rgba(0,0,0,0.5)] z-10 pointer-events-none flex items-center justify-center -ml-[2px]"
                  style={{ left: `${sliderPos}%` }}
                >
                  <div className="w-8 h-8 md:w-10 md:h-10 bg-white rounded-full flex items-center justify-center shadow-[0_0_15px_rgba(0,0,0,0.3)] text-[#ff3e5e]">
                     <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="m15 18 6-6-6-6"/><path d="m9 6-6 6 6 6"/></svg>
                  </div>
                </div>
                ...
                <div className="absolute bottom-6 left-6 right-6 text-center pointer-events-none z-10">
                   <h4 className="text-xl sm:text-2xl md:text-3xl font-black text-white mb-1 drop-shadow-md">CRICIÚMA <span className="text-[#fce315]">E</span> IÇARA</h4>
                   <p className="text-white/90 font-semibold uppercase tracking-wider text-xs sm:text-sm drop-shadow-md">A Rádio que toca a nossa região</p>
                </div>
              </div>
            </div>
          </div>
